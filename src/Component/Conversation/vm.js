import { Channel, Setting, ChannelTypeGroup, ChannelTypePerson, ConversationAction, WKSDK, Message, MessageContent, MessageStatus, Subscriber, Conversation, MessageExtra, CMDContent, PullMode, MessageContentType } from "wukongimjssdk";
...
export default class ConversationVM extends ProviderListener {
    ...
    loading = false; // 消息是否加载中
    channel = null; 
    messages = [] // 消息集合 
    currentConversation = {}; // 当前最近会话
    messagesOfOrigin = []; // 原始消息集合（不包含时间消息等本地消息）
    browseToMessageSeq = 0; //  已经预览到的最新的messageSeq
    initLocateMessageSeq = 0; // 初始定位的消息messageSeq 0为不定位
    shouldShowHistorySplit = false; // 是否应该显示历史消息分割线
    _editOn = false; // 是否开启编辑模式
    orgUnreadCount = 0; // 原未读数量
    _unreadCount = 0; // 当前未读消息数量

    pullupHasMore = false; // 上拉是否有更多
    pulldownFinished = false; // 下拉完成
    messageContainerId = "viewport"; // 消息容器的ID
    sendQueue = new Map(); // 发送队列
    _needSetUnread = false; // 是否需要设置未读数量

    typingListener = () => {}; // 输入中监听
    messageListener = () => {};  // 消息监听
    cmdListener = () => {};  // cmd消息监听
    messageStatusListener = () => {};  // 消息状态监听
    lastMessage = {}; // 此会话的最后一条最新的消息
    lastLocalMessageElement = null; // 最后一条消息的dom元素
     _showScrollToBottomBtn = false; // 是否显示底部按钮
    subscribers = []

    fileDragEnter = false; // 文件拖拽上传（拖进来了）
    fileDragLeave = false // 文件拖拽上传（拖离开了）

     _selectMessage = undefined; // 右键选中的消息

    selectUID = ''; // 点击头像的用户uid

    _currentReplyMessage // 当前回复的消息

    onFirstMessagesLoaded = () => {} // 第一屏消息已加载完成
    ...
    sendMergeforward(toChannels) {
        let users = new Array();

        let checkedMessages = this.getCheckedMessages().map((messageWrap) => {
    ...
    async deleteMessages(deletedMessages) {
    ...
    async deleteMessagesFromLocal(deletedMessages) {
    ...
    removeSendingMessageIfNeed(clientSeq, channel) {
    ...
    didMount() {
        ...
        // cmd监听
        this.cmdListener = (message) => {
            const cmdContent = message.content;
        ...
        if (this.channel.channelType === ChannelTypeGroup) {
            ...
            WKSDK.shared().channelManager.addSubscriberChangeListener((channel) => {
        ...
        // 输入中监听
        this.typingListener = (channel, add) => {
    ...
    // 移除“输入中”这条消息
    removeTypingMessage(notify = true) {
    ...
    // 更新消息状态
    updateMessageStatusBySendAck(ackPacket) {
    ...
    // 更新消息扩展数据
    updateMessageByMessageExtras(messageExtras) {
    ...
    // 通过clientMsgNo获取消息对象
    findMessageWithClientMsgNo(clientMsgNo) {
        if (!this.messages || this.messages.length <= 0) {
            return
        }
        for (let i = this.messages.length - 1; i >= 0; i--) {
    ...
    // 通过messageID获取消息对象
    findMessageWithMessageID(messageID) {
        if (!this.messages || this.messages.length <= 0) {
            return
        }
        for (let i = this.messages.length - 1; i >= 0; i--) {
    ...
    // 通过messageSeq获取消息对象
    findMessageWithMessageSeq(messageSeq) {
        if (!this.messages || this.messages.length <= 0) {
            return
        }
        for (let i = this.messages.length - 1; i >= 0; i--) {
    ...
    // 根据情况更新最后一条消息
    updateLastMessageIfNeed(message) {
        let change = false
        if (!this.lastMessage) {
    ...
    // 获取第一屏消息
    requestMessagesOfFirstPage(lcateMessageSeq, stateCallback) {
    ...
    // 同步消息
    async syncMessages(initMessageSeq, stateCallback) {
        this.loading = true
        this.notifyListener()
    ...
    // 刷新消息列表并定位到某条消息
    refreshAndLocateMessages(messages, locateMessage, scrollBottom, callback) {
    ...
    // 刷新消息列表
    refreshMessages(messages, callback) {
    ...
    toMessageWraps(messages) {
        const messageWraps = new Array()
        if (messages) {
            for (const message of messages) {
